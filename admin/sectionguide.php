    ...
    <main>
        ...
        <h2>The Questions Table</h2>
            ...
            <ul>
                ...
                <ere><strong>Multiple</strong> - These are categories which potentially repeat, like number of branches, number of board members, and counts of library positions.  Just like a query, these require an entry in the "Query" blank.  However, the information put here is not the SQL query itself.  Because the looping through these entries is complicated, they are handled by functions in the <em>functions.php</em> file.  There are three values entered here separated by the pipe (|) character: group number, function name, and variable name.  The group number is to differentiate groupings of questions and is largely arbitrary, although it does need to be unique for each group.  The named function is defined in the <em>functions.php</em> file and if you need to change the nature of the data being collected, that's where you need to look.  The final value is the name of the variable that is returned from the function that is to be used for this field.</li>
            </ul>
            <p>The formats are:</p>
            <ul>
